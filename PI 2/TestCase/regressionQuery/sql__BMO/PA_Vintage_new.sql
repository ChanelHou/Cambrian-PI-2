...
select  fi_cat, product_name,  vintage_quarter,quarter_age,
sum(delinquencyBalance)/sum(total_balance)*100 avg_delinquencyRate,
sum(total_balance) total_balance,
if(sum(total_credit)<>0,sum(total_balance)/sum(total_credit)*100,null) avg_utilization,
sum(total_balance)/sum(trade_count) avg_balance,
...
from  (
  ...
  select  concat( cast( floor(vintage_month/100) as string),'Q',cast( ceiling(mod(vintage_month,100)/3) as string)) vintage_quarter,mod(vintage_month,100)+account_age - 3 *ceiling(mod(vintage_month,100)/3) quarter_age ,product_name, 
 ...
 if(active_flag='Y',total_credit,0) active_total_credit
  ...
  where  vintage_month between ${fromMonth} and ${toMonth}) t
group by  fi_cat, product_name,  vintage_quarter,quarter_age  
having quarter_age >=0 
and quarter_age <=${age}  
order by  fi_cat, product_name,  vintage_quarter,quarter_age ;
...
group by year_month,vintage_month,   fi_cat, product_name   
 ...
 --chart 54 by quarter
 select 
year_month, vintage_year,vintage_quarter,fi_cat,product_name , 
 sum(orig_cons_age_total)/ sum(orig_cons_age_count) avg_orig_age, 
sum(total_balance) total_balance,
sum(total_balance)/sum(trade_count) avg_balance,
sum(total_credit)/sum(trade_count) avg_limit ,  
...
from  (
  ...
  select  year_month,  floor(vintage_month/100) vintage_year, ceiling(mod(vintage_month,100)/3) vintage_quarter,
        ...
        product_name,
          ...
          trade_count, total_balance, total_credit, total_score, 
		  score_count, orig_total_score, orig_score_count, orig_cons_age_total, orig_cons_age_count, 
 if(payment_status='90+', total_balance,0) delinquencyBalance    
...
group by year_month, vintage_year,vintage_quarter,fi_cat,product_name 
order by year_month, vintage_year,vintage_quarter,fi_cat,product_name ;


 
--chart 54 trade count , avg limit by month 

select    vintage_month,
         case when fi_name = "BANK OF MONTREAL" then "BMO"
              when  peer_id =3 then 'Peer'
              else 'Other'
         end fi_cat,   product_name, sum(total_credit) , sum(total_credit)/um(trade_count) avgLimit,
...
select year_month,year,quarter,fi_cat, product_name, sum(trade_count) trade_count ,sum(total_credit)/sum(trade_count) avgLimit from 
(select year_month,  
         case when fi_name = "BANK OF MONTREAL" then "BMO"
              when  peer_id =3 then 'Peer'
              else 'Other'
         end fi_cat,   product_name, 
		 case when mod(year_month,100) = 1 then floor(year_month/100) - 1  else floor(year_month/100) end year ,
		 case when mod(year_month,100) = 1 then 4 else (mod(year_month,100)-1)/ 3 end quarter ,
		 trade_count,total_credit,account_age
	
from  pi2_development_db.trade_vintages_o r
where  mod(year_month,100) in ( 1,4,7,10)  
and account_age in ( 1,2,3)
) a 

 group by  year_month,year,quarter,fi_cat, product_name
 order by  year_month,year,quarter,fi_cat, product_name ; 
...
select  bmo.year_month, bmo.vintage_month, bmo.fi_cat,  bmo.product_name, bmo.orig_age_cat,
